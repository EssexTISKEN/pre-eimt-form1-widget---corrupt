ZOHO.embeddedApp.on("PageLoad", async function (data) {
    ...
    /* ============================================================
       DRAG & DROP (version MULTIFICHIERS – PATCH APPLIQUÉ)
    ============================================================ */
    function initDropzones() {
        document.querySelectorAll(".dropzone").forEach(zone => {
            ...
            // 🎯 PATCH : Fusion des fichiers existants + des nouveaux
            zone.addEventListener("drop", (e) => {
                ...
                // 1) Ajouter les fichiers déjà présents dans le input
                if (input.files && input.files.length > 0) {
                    for (const f of input.files) dt.items.add(f);
                }

                // 2) Ajouter les nouveaux fichiers déposés
                for (const f of e.dataTransfer.files) {
                    dt.items.add(f);
                }

                // 3) Fusion écrite dans input.files
                input.files = dt.files;

                updateList();
            });

            // Via bouton parcourir
    ...
    $("btn_submit").onclick = async () => { 
        ...
        const payload = { 
            ...
            Avantages_sociaux: $("Avantages_sociaux").value || "", 
